...
library(readxl)
twix <- read_excel("/Users/lahlo/Downloads/ChampionnatFR_T.xlsx")
twix <- twix[,2:11]
colnames(twix) <- c("moyenne","medianedegree","ecartypedegree","betweeness","medianebetw","ecartypebetw","tabdensity","tabtransitivity","tabdiameter","nbpoints")

tabfinal <- rbind(indicateurs1,twix)

reg.mult2 <- lm(nbpoints~.,tabfinal)

summary(reg.mult2)

plot(reg.mult2,which=c(1,2),label.size=2)  


library(leaps)

select2<-regsubsets(nbpoints~., data=tabfinal, nvmax=10, method="forward")
plot(select2,scale="bic")

library(MASS)

select_aic2=stepAIC(lm(nbpoints~.,data=tabfinal), trace=TRUE)

summary(select_aic2)

reg.simple1 <- lm(nbpoints~moyenne,tabfinal)
summary(reg.simple1)



reg.simple2 <- lm(nbpoints~medianedegree,tabfinal)
summary(reg.simple2)
plot(reg.simple2,which=c(1,2),label.size=2)  


reg.simple3 <- lm(nbpoints~medianebetw,tabfinal)
summary(reg.simple3)
plot(reg.simple3,which=c(1,2),label.size=2)  
